...
- **macOS 기본**: Chrome 경로가 하드코딩이지만, `MAILMAN_CHROME_PATH` 환경변수로 다른 OS도 가능
...
- Codex CLI: `mailman` 커맨드로 로컬 Playwright 런타임 사용
- Codex 샌드박스: marketplace plugin이 snapshot driver로 import

자세한 건 [docs/codex.md](docs/codex.md) 참고.

**Q. 팀 배포용 Codex plugin도 있나요?**

A. 네. 독립 배포 패키지는 `packages/mailman-sandbox-plugin/` 에 있습니다.
...
| `spaces.{별칭}.mentionFilter`     | 멘션 필터 문자열             | 설정하면 해당 문자열이 포함된 메시지만 수집. 생략 시 전체 수집       |
...
- Claude Code: `/mailman`
- Codex CLI: `mailman`
- Codex 샌드박스: 설치된 `Mailman Sandbox` plugin의 `mailman` skill 사용
...
- [scripts/mailman/README.md](scripts/mailman/README.md) — 런타임 환경변수, 설정, snapshot driver
- [docs/claude.md](docs/claude.md) — Claude Code 연결
- [docs/codex.md](docs/codex.md) — Codex 개요
- [docs/codex-install.md](docs/codex-install.md) — Codex 설치
- [docs/codex-usage.md](docs/codex-usage.md) — Codex 사용
